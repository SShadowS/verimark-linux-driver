...
**Status: works on real hardware.** The driver builds and links into upstream
libfprint 1.94.10 and enrolls + verifies + unlocks the VeriMark Desktop
(`047d:00f2`) through `fprintd` — confirmed on Fedora 44. `verimark.c` wires every
layer together (P0-P6 of `driver/PORTING-PLAN.md`): USB open/claim, the P1/P2
...
`verimark-moc.c`. All wire protocol and crypto were solved and prototyped in Python
(`prototype/p2_moc.py`, findings/49, 51) before this port; nothing here is new RE.
...
- **On hardware** — enroll (coverage `0x01`→`0x7f`), verify/identify (single tap),
  ...
  libfprint 1.94.10).
- **`driver/tests/`** — glib+libcrypto-only unit tests for the crypto core, the TLS
...
- **The wire protocol** — independently proven end-to-end from Linux by the Python
...
GUID bridge — `STORAGE_LIST` is currently off), `delete`/`clear_storage` on-device
exercise, and an `EC_KEY`→EVP crypto-core migration. See the top-level README's
