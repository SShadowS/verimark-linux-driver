...
| `install-verimark.sh` | Points the **system** `fprintd` at the build produced above for on-device testing, non-destructively and reversibly: udev rule + a systemd `LD_LIBRARY_PATH` drop-in on `fprintd.service` (never overwrites `/usr/lib*/libfprint-2.so*`, never `meson install`s). `--uninstall` reverts fully; safe to re-run either direction. |
